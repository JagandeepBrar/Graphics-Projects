...
bool Scene::intersect( const ray& r, isect& i ) const
{
	...
	if(traceUI->getOctreeStatus()) {
		found = octreeIntersect(r,i);
	} else {
		found = bruteforceIntersect(r,i);
	}
...
bool Scene::octreeAssemble() {
	octree = std::unique_ptr<OcTree<Geometry>>(new OcTree<Geometry>(objects,traceUI->getOctreeDepth()));
	for(int i=0; i<(int)objects.size(); i++) {
		if(objects[i]->isTrimeshObject()) {
			//Need to make a trimesh octree as well
		}
	}
	return true;
}

bool Scene::octreeIntersect(const ray &r, isect &i) const {
	return octree->intersect(r,i);
}

bool Scene::bruteforceIntersect(const ray &r, isect &i) const {
